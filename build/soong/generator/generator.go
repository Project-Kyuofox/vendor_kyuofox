...
func init() {
	android.RegisterModuleType("kyuofox_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = kyuofoxExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = kyuofoxExpandVariables(ctx, dep_file)
	...
	cmd := kyuofoxExpandVariables(ctx, String(g.properties.Cmd))
